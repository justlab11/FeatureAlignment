...
os.rename(f"models/contrast_body_plain+skips_{best_temp}.pt", f"models/contrast_body_plain+skips.pt")

# train contrastive learning classifier
num_class_epochs = 20

contrast_body = TinyCNN_Headless()
contrast_body.load_state_dict(torch.load(f"models/contrast_body_plain+skips.pt", weights_only=True))
