...
class HEIFFolder(Dataset):
    ...
    def validate_dataset(self):
        """
        Run this after __init__ to catch errors early
        This function was generated by claude to validate the dataset
        """
        ...
        # Check class sample indices
        for class_idx, indices in self.class_samples.items():
            for i in indices:
                if i >= len(self.samples):
                    raise IndexError(f"Class {class_idx} contains invalid index {i}")

        print(f"Dataset validated: {len(self.samples)} samples, {len(self.class_to_idx)} classes")
...
class IndexedDataset(Dataset):
    ...
    def _validate_indices(self):
        """
        Given by Claude to check for indexing errors
        """
        max_valid = len(self.dataset) - 1
        invalid_indices = [i for i in self.indices if i > max_valid]
        
        # Add negative index check
        negative_indices = [i for i in self.indices if i < 0]
        invalid_indices.extend(negative_indices)
        ...
        # Add uniqueness check (optional but recommended)
        if len(set(self.indices)) != len(self.indices):
            duplicates = list(set([x for x in self.indices 
                                if self.indices.count(x) > 1]))
            raise ValueError(f"Duplicate indices detected: {duplicates[:5]}...")

    def _build_class_samples(self):
        class_samples = {}
        for subset_idx, original_idx in enumerate(self.indices):
            # Use dataset.__getitem__ instead of direct samples access
            _, label = self.dataset[original_idx]  # Forces index validation
            label = int(label)
            
            if label not in class_samples:
                class_samples[label] = []
            class_samples[label].append(subset_idx)  # Store SUBSET positions
    ...
    def __getitem__(self, idx):
        if idx >= len(self.indices):
            raise IndexError(
                f"Requested subset index {idx} but only {len(self.indices)} exist"
            )
        ...
        if original_idx >= len(self.dataset):
            raise IndexError(
                f"Original index {original_idx} invalid "
